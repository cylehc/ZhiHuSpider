...
//        urlQueue.put("https://www.zhihu.com/people/yao-cheng-46");
        urlQueue.put("https://www.zhihu.com/people/chasegalaxy");
    ...
    private static void setUserInfo1(Element userUrlContent, ZhiHuUserBean user) {
        Elements elements = userUrlContent.select(".ProfileHeader-contentBody");
        System.out.println("spriderService.setUserInfo1");
    }
    public static void crawler(String tmp) {
        //解析网页
        Element userUrlContent = Jsoup.parse(getFromUrl(tmp));
        ...
        //userUrlContent.select(".App-main").select("div[itemprop=people]");

        //setUserInfo1(userUrlContent, user);
        if (size == 2) {
        ...
        Elements metaData = userUrlContent.select(".App-main").select("div[itemprop=people]");
        String sexString = metaData.select("meta[itemprop=gender]").first().attr("content");
...
//                    String answersNum = userUrlContent.select(".ProfileMain-header").first().select("li").get(1).text().substring(2);
        user.setAnswersNum(metaData.select("meta[itemprop=gender]").first().attr("zhihu:answerCount"));
...
//                    String starsNum = starsNumString.substring(starsNumString.indexOf("得") + 2, starsNumString.indexOf("次") - 1);
        ...
        user.setStarsNum(metaData.select("meta[itemprop=gender]").first().attr("zhihu:voteupCount"));
        user.setThxNum(metaData.select("meta[itemprop=gender]").first().attr("zhihu:thankedCount"));

        //关注的人
        user.setFollowingNum(userUrlContent.select(".NumberBoard-itemValue").get(0).text());

        //关注者数量
        user.setFollowersNum(userUrlContent.select(".NumberBoard-itemValue").get(1).text());
